...
int login_qq(struct QQ_DATA_INFO *buf, mylink *head)
{
    ...
    fd = open(buf->srcname, O_WRONLY);      /*获取登陆者名字,以只写方式打开以其名字命名的私有管道*/
    mylink node = make_node(buf->srcname,fd);   /*利用用户名和文件描述符创建一个节点*/
    mylink_insert(head, node);              /*将创建的节点插入链表*/
...
void transfer_qq(struct QQ_DATA_INFO *buf, mylink *head)
{
    ...
    if(p == NULL)
    {
        struct QQ_DATA_INFO lineout;               /*目标用户不在,封装3号数据包*/
        lineout.protocal = 3;
...
int main(void)
{
   ...
   struct QQ_DATA_INFO dbuf;                            /*定义数据包结构对象*/

   if(access(SERVER_PROT, F_OK) != 0)                   /*判断共有管道是否存在,不存在则创建*/
   {
